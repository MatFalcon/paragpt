    ...
    def compute(self, value, date_ref=False, currency=None):
        ...
        for line in self.line_ids:
            ...
            if line.value != 'instalment' and amt:
                ...
                instalment_amount=float_round((amt/count), precision_rounding=rounding,rounding_method='HALF-UP')
                ...
                while count > 0:
                    ...
                    if count==1:
                        instalment_amount_final=float_round(valor-total, precision_rounding=rounding,rounding_method='HALF-UP')
                        print(instalment_amount_final)
                        result.append((fields.Date.to_string(next_date), instalment_amount_final))
                    else:
                        result.append((fields.Date.to_string(next_date), instalment_amount))
                    total += instalment_amount
                    count -= 1
                amount -= amt
