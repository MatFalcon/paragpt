...
class account_check_dreject(models.TransientModel):
    ...
    def make_invoice(self, invoice_type, check):
        ...
        if invoice_type == 'in_invoice':
            ...
            account_id = (
                check.voucher_id.journal_id.default_account_id.id)
        else:
            ...
            if check.state == 'handed':
                ...
                account_id = (
                    deposit_move.journal_id.default_account_id.id)

        if not journal:
            raise ValidationError(_('No journal for rejection in company %s') %
    ...
    def make_expenses_move(self, check):
        ...
        if check.state == 'handed':
            ...
            deposit_move = check.deposit_account_move_id
            account_id = deposit_move.journal_id.default_account_id.id
        move.line_id.with_context({}).create({
            'name': name,
            'centralisation': 'normal',
            'account_id': account_id,
