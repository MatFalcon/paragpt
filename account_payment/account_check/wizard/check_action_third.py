...
class account_check_third_action(models.TransientModel):
    _name = 'account.check.third.action'

    
    def _get_company_id(self):
        active_ids = self._context.get('active_ids', [])
        checks = self.env['account.check.third'].browse(active_ids)
        company_ids = [x.company_id.id for x in checks]
        if len(set(company_ids)) > 1:
            raise ValidationError(_('All checks must be from the same company!'))
    ...
    def validate_action(self, action_type, check):
        ...
        #     else:   # third
        #         raise ValidationError(_('You can not debit a Third Check.'))
        ...
        #     else:   # issue
        #         raise ValidationError(_('You can not return a Issue Check.'))
        # elif action_type == 'conciliar':
        #     if check.state != 'handed':
        #         raise ValidationError(_('No se pueden conciliar cheques cuyo estado no sea el de Entregado'))
        return True
    ...
    def action_confirm(self):
        ...
        for check in self.env['account.check.third'].browse(
