...
class AccountPaymentRegisterInherit(models.TransientModel):
    ...
    def _create_payment_vals_from_wizard(self, batch_result):
        ...
        factura = self.line_ids.mapped('move_id').filtered(lambda x: x.move_type in ['out_invoice', 'in_invoice'])
        factura = factura[0] if factura else None
        ...
        if factura:
            ...
            _logger.info("Asignando fecha_factura: %s y numero_factura: %s al payment_vals", fecha_factura, numero_factura)
        else:
            payment_vals.update({
                'fecha_factura': None,
                'numero_factura': None,
            })
            _logger.info("No se encontró factura asociada, payment_vals fecha_factura y numero_factura se asignan como None")
