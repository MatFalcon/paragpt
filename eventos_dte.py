...
class EventosDte(models.Model):
    ...
    tipo_comprobante = fields.Selection(selection=[('1', 'Factura electrónica'),
                                                   ...
                                                   ('9', 'No definido')
                                                   ], string="Tipo Comprobante", compute="set_comprobante")
    ...
    state = fields.Selection(selection=[('borrador', 'Borrador'), ('aprobado', 'Aprobado'), ('rechazado', 'Rechazado')],
                             default='borrador')
    ...
    def generar_xml(self, certificado):
        ...
        rGesEve = lxml.etree.Element('rGesEve',
        ...
        elif self.tipo=='nominacion':
            rGeVeNom = lxml.etree.SubElement(gGroupTiEvt, 'rGEveNom')
            if not self.cdc:
                raise ValidationError('Se necesita el cdc para este evento')
            lxml.etree.SubElement(rGeVeNom, 'Id').text = str(self.cdc)
            if len(self.mOtEve) < 5:
                raise ValidationError('El campo de motivo de evento debe ser mayor a 5 caracteres')
            lxml.etree.SubElement(rGeVeNom, 'mOtEve').text = str(self.mOtEve)
            lxml.etree.SubElement(rGeVeNom, 'iNatRec').text = str(int(self.dNomRec.naturaleza_receptor))
            lxml.etree.SubElement(rGeVeNom, 'iTiOpe').text = str(self.dNomRec.tipo_operacion)
            lxml.etree.SubElement(rGeVeNom, 'cPaisRec').text = str(self.dNomRec.country_id.codigo_set)
            lxml.etree.SubElement(rGeVeNom, 'dDesPaisRe').text = str(self.dNomRec.country_id.name)
            if str(int(self.dNomRec.naturaleza_receptor)) == '1':
                if self.dNomRec.company_type == 'person':
                    # gDatRec.setdefault('iTiContRec',1)
                    lxml.etree.SubElement(rGeVeNom, 'iTiContRec').text = str(1)
                else:
                    lxml.etree.SubElement(rGeVeNom, 'iTiContRec').text = str(2)
                lxml.etree.SubElement(rGeVeNom, 'dRucRec').text = str(self.dNomRec.ruc)
                lxml.etree.SubElement(rGeVeNom, 'dDVRec').text = str(self.dNomRec.dv)
            else:
                ...
                tipo_tipo_doc = dict(value_tipo_doc)[int(self.dNomRec.tipo_documento_receptor)]
                lxml.etree.SubElement(rGeVeNom, 'iTipIDRec').text = str(int(self.dNomRec.tipo_documento_receptor))
                lxml.etree.SubElement(rGeVeNom, 'dDTipIDRec').text = tipo_tipo_doc
                lxml.etree.SubElement(rGeVeNom,
                                      'dNumIDRec').text = self.dNomRec.nro_documento or self.dNomRec.digitointer or self.dNomRec.ci or self.dNomRec.rucdv
            lxml.etree.SubElement(rGeVeNom, 'dNomRec').text = self.dNomRec.name
            if self.dNomRec.nombre_fantasia:
                lxml.etree.SubElement(rGeVeNom, 'dNomFanRec').text = self.dNomRec.nombre_fantasia
                lxml.etree.SubElement(rGeVeNom, 'dDirRec').text = self.dNomRec.street
                lxml.etree.SubElement(rGeVeNom, 'dNumCasRec').text = self.dNomRec.nro_casa
            try:
                refence_geo_id = self.dNomRec.reference_geo_id
                lxml.etree.SubElement(rGeVeNom, 'cDepRec').text = str(
                    refence_geo_id.reference_geo_district_id.refence_geo_id.codigo)
                lxml.etree.SubElement(rGeVeNom, 'dDesDepRec').text = str(
                    refence_geo_id.reference_geo_district_id.refence_geo_id.name)
                lxml.etree.SubElement(rGeVeNom, 'cDisRec').text = str(
                    refence_geo_id.reference_geo_district_id.codigo)
                lxml.etree.SubElement(rGeVeNom, 'dDesDisRec').text = str(
                    refence_geo_id.reference_geo_district_id.name)
                lxml.etree.SubElement(rGeVeNom, 'cCiuRec').text = str(refence_geo_id.codigo)
                lxml.etree.SubElement(rGeVeNom, 'dDesCiuRec').text = str(refence_geo_id.name)
            except:
                a = 1
            if self.dNomRec.phone:
                lxml.etree.SubElement(rGeVeNom, 'dTelRec').text = self.dNomRec.phone
                lxml.etree.SubElement(rGeVeNom, 'dCelRec').text = self.dNomRec.mobile
                lxml.etree.SubElement(rGeVeNom, 'dEmailRec').text = self.dNomRec.email
            if self.dNomRec.ref:
                lxml.etree.SubElement(rGeVeNom, 'dCodCliente').text = self.dNomRec.ref
    ...
    def parsear_response(self, response):
        ...
        if response.status_code == 200:
            ...
            if res_tex.find('html') < 0 and res_tex.find('rRetEnviEventoDe') > 0:
                ...
                rResEnviLoteDe = res_tex[res_tex.find('rRetEnviEventoDe') - 1:res_tex.rfind('rRetEnviEventoDe') + len(
                    'rRetEnviEventoDe') + 1]
