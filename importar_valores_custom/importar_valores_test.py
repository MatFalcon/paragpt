...
def convertir_fecha(fecha):
    if fecha:
        if isinstance(fecha, datetime.datetime):
            return fecha.date()  # Si ya es datetime, devolvé solo la fecha
        return str(datetime.datetime.strptime(str(fecha), '%Y-%m-%d').date())
    return False
...
def obtener_novedades_desde_BD():
    ...
    for r in results:
        ...
        if r.get('FechaOperacion'):
            fecha_operacion = datetime.datetime.strptime(r.get('FechaOperacion'), '%Y-%m-%d')
        else:
            fecha_operacion = False

        if r.get('FechaVencimiento'):
            fecha_vencimiento = datetime.datetime.strptime(r.get('FechaVencimiento'), '%Y-%m-%d')
        else:
            fecha_vencimiento = False

        if fecha_vencimiento and fecha_operacion:
            plazo = (fecha_vencimiento - fecha_operacion).days
        else:
            plazo = 0
        currency_id = 2 if r.get('MonedaID') == 2 else 155
        obj = {
            'id_vvalores': int(r.get('OperacionCarteraNumero')),
            'cantidad': int(r.get('Cantidad')),
            'total': False,
            'total_iva': False,
            'subtotal': False,
            ...
            'volumen_gs': float(r.get('VolumenGS') if moneda_id == 1 else r.get('SubTotal')),
            'volumen_gs_usd': float(r.get('VolumenGS')),
