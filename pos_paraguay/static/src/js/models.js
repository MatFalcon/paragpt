...
/*Para buscar por nro de factura*/
patch(TicketScreen.prototype, {
    ...
    getInvoiceNumber(order) {
        console.log("Nro");
        console.log("ORDENNNNNNNNNNNNNNN");
        console.log(order.account_move);
        console.log(order.account_move && order.account_move.name)
        console.log(order)
        return order.account_move?.name || "";
    },
    /*Sobreescribo directamente la funcion para no complocarme con la herencia*/
    _getSearchFields() {
        const fields = {
        /* Con esto sale como ocpcion en nro de fact */
        INVOICE_NUMBER: {
                repr: (order) => order.account_move && order.account_move.name,
                displayName: _t("Nro Factura"),
