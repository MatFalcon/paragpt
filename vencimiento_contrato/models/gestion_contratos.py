...
class GestionContratos(models.Model):
    ...
    fecha_inicio = fields.Date(string="Fecha de inicio", required=True, default=lambda self: fields.Date.today(),tracking=True)
    fecha_fin = fields.Date(string="Fecha de fin", required=True, default=lambda self: fields.Date.today(),tracking=True)
    fecha_fin_aux = fields.Date(string="Fecha de fin auxiliar")
    state = fields.Selection(string="Estado",
                             selection=[('vencido', 'Vencido'), ('vigente', 'Vigente'), ('por_iniciar', 'Por iniciar'),('no_vigente','No vigente')],
                             compute="vigencia", store=True)
    contrato_indefinido = fields.Boolean(string="Contrato indefinido")


    def set_no_vigente(self):
        for rec in self:
            rec.write(
                {
                    'state': 'no_vigente'
