...
class FondoGarantiasPeriodos(models.Model):
    ...
    rendimiento_mes = fields.Float(string="Rendimiento del mes")
    capital_vencido_mes = fields.Float(string="Capital vencido del mes")
    egresos_mes = fields.Float(string="Egresos del mes")


    @api.depends('fecha_incio','fecha_fin')
    @api.onchange('fecha_incio','fecha_fin')
    def computeValuesByDateRange(self):
        if self.fecha_fin and self.fecha_inicio:
            self.compute_rendimiento_mes()
            self.compute_capital_vencido_mes()
            self.compute_egresos_mes()
    ...
    def compute_rendimiento_mes(self):
        for i in self:
            ...
            if registros:
                i.rendimiento_mes = sum(registros.mapped('importe_valorizado'))
            else:
                i.rendimiento_mes = 0
    ...
    def compute_capital_vencido_mes(self):
        for i in self:
            registros = self.env['pbp.inversion_fondo_garantias'].search([('fecha_vencimiento', '>=', i.fecha_inicio),
                                                                          ...
                                                                          ('tipo','=','capital')])
            if registros:
                i.capital_vencido_mes = sum(registros.mapped('importe_valorizado'))
            else:
    ...
    def compute_egresos_mes(self):
        for i in self:
            diario_fg = self.env['account.journal'].search(
                [('diario_fg', '=', True), ('currency_id', '=', i.currency_id.id)])
            if diario_fg:
                lineas = self.env['account.move.line'].search([('account_id', '=', diario_fg.default_account_id.id),
                                                               ('date', '>=', i.fecha_inicio),
                                                               ('date', '<=', i.fecha_fin)])
    ...
    def generar_registros(self):
        for i in self:
            ...
            if not i.fecha_inicio or not i.fecha_fin or i.fecha_inicio > i.fecha_fin:
                ...
                registro_mes_anterior = self.env['pbp.fondo_garantias_periodos'].search(
                    [('currency_id', '=', i.currency_id.id),
...
class FondoGarantias(models.Model):
    ...
    participacion_sobre_rendimiento = fields.Float(string="Participación sobre rendimiento", compute="compute_participacion_sobre_rendimiento")
    ...
    def compute_cobro_fg_aporte_cbsa(self):
        for i in self:
            lineas = self.env['account.move.line'].search([('move_id.asiento_fg', '=', True),
                                                           ('date', '>=', i.periodo_id.fecha_inicio),
                                                           ('date', '<=', i.periodo_id.fecha_fin),
                                                           ('partner_id', '=', i.partner_id.id)])
            if lineas:
                i.cobro_fg_aporte_cbsa = sum(lineas.filtered(lambda x: x.amount_currency > 0
                                                                       and x.currency_id == i.periodo_id.currency_id).
                                             mapped('amount_currency'))
            else:
                i.cobro_fg_aporte_cbsa = 0

    @api.onchange('periodo_id.rendimiento_mes')
    @api.depends('periodo_id.rendimiento_mes')
    def compute_participacion_sobre_rendimiento(self):
        for i in self:
            porcentaje = 0
            if i.periodo_id.rendimiento_mes > 0:
                porcentaje = abs(i.por_participacion_sobre_total)*i.periodo_id.rendimiento_mes
            i.write({'participacion_sobre_rendimiento': porcentaje})

    @api.onchange('por_participacion_sobre_total', 'periodo_id.egresos_mes', 'periodo_id.capital_vencido_mes')
    @api.depends('por_participacion_sobre_total', 'periodo_id.egresos_mes', 'periodo_id.capital_vencido_mes')
    def compute_participacion_egreso_sobre_rendimiento(self):
        for i in self:
            total = i.por_participacion_sobre_total * (i.periodo_id.egresos_mes - i.periodo_id.capital_vencido_mes)
