            ...
            if budget:
                ...
                for grupo_padre, lista_hijos in diccionario_grupos.items():
                    ...
                    if len(ingresos) > 0 and len(egresos) > 0 and campo != 'percentage':
                        while mes < 12:
                            egresos_mes = egresos[mes]['monto']
                            if egresos_mes < 0:
                                egresos_mes = egresos_mes * (-1)
                            monto = ingresos[mes]['monto'] - egresos_mes
                                ...
                                for ca in cuentas_analiticas:
                                    ...
                                    for l in lineas_budget.filtered(lambda x: x.analytic_account_id == ca):
                                        if campo == 'planned_amount':
                                            ...
                                            if planned_amount < 0:
                                                planned_amount = l.planned_amount * -1
                                            rightAndWrite(planned_amount, numerico)
                                            suma_cuenta = suma_cuenta + l.planned_amount
                                        elif campo == 'practical_amount':
                                            practical_amount = l.practical_amount
                                            if practical_amount < 0:
                                                practical_amount = practical_amount * -1
                                            rightAndWrite(practical_amount, numerico)
