...
def getLiquidacion(Sueldo_Prom_Diario, preaviso_Correspondido, preaviso_Dado, vacaciones_Causada,
    ...
    if antiguedadOj:
        ...
        if (antAnho > 0):
            liq_Indemniz = ((dias_indem * antAnho) * Sueldo_Prom_Diario)
        else:
    ...
    if perprueba:
        ...
        if not posible_preaviso:
            liq_Preaviso = 0
        liq_SubTot_1 = liq_diasTrabaj + liq_Vacaciones + liq_VacacionesAtras + liq_diasVacPropor + liq_vacacionesCausada - liq_Preaviso
        liq_indemnizacion = 0
        if justificada:
