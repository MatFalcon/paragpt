...
class CarteraInversion(models.Model):
    ...
    credit_account_id = fields.Many2one('account.account', string='Acreedor')
    ...
    debit_account_id = fields.Many2one('account.account', string='Deudor')
    ...
    def generar_asientos(self, records=None):
        if records:
            carteras = records.read((set(self.env['pbp.cartera_inversion']._fields)))
