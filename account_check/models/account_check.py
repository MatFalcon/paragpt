...
class account_check(models.Model):
    ...
    type = fields.Selection([('third_check', 'Cheque de Terceros'), ('issue_check', 'Cheques Propios')],
                            compute='obtener_tipo', string='Tipo',default='issue_check', store=True, compute_sudo=True)
    journal_id = fields.Many2one('account.journal',compute='obtener_tipo', string='Diario',store=True, compute_sudo=True)
        ...
        if self.number:
            if self.checkbook_id:
                if self.number > self.checkbook_id.range_to:
                    raise ValidationError('El cheque nro "%s" es mayor que del ultimo numero  de la chequera %s, con numero de finalizacion= %s. Favor verificar'%(str(self.number),self.checkbook_id.name, str(self.checkbook_id.range_to)))
                if self.number < self.checkbook_id.range_from:
                    raise ValidationError('El cheque nro "%s" es menor que del primer numero  de la chequera %s, con numero de inicial= %s. Favor verificar'%(str(self.number),self.checkbook_id.name, str(self.checkbook_id.range_from)))
            else:
                raise ValidationError('No posee chequera asociada al cheque, favor agregar')
