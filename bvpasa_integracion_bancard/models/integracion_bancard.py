...
class IntegracionBancard(models.Model):
    ...
    def obtenerReportes(self):
        ...
        # _logger.warning('url es %s', url)
        # _logger.warning('pass es %s', password)
        # _logger.warning('username es %s', username)
        # _logger.warning('file_path es %s', file_path)

        url = url + '?from_date=2024-01-01'

        # _logger.warning('url path fecha es %s', url)

        response = requests.get(url, auth=(username, password))
        # _logger.warning('response es %s', response)

        if response.status_code == 200:
            res = response.json()
            # _logger.warning('res de obt reportes bancard es %s', res)
            ...
            for l in lines:
                ...
                with open(file_path, newline='', encoding='latin-1') as csvfile:
                    ...
                    for row in reader:
                        if c != 0:
                            moneda = self.env['res.currency'].search([('name', '=', str(row[10]))]) if len(row) > 9 else False
                            if not moneda:
                                break
                            moneda = moneda.id
    ...
    def FacturarCoincidentes(self):
        ...
        for b in r_bancard:
            c = r_campus.filtered(lambda x:x.ticket_number == b.nro_transaccion and
                                           x.fecha_transaccion >= datetime.datetime.strptime('01/01/2024', '%d/%m/%Y'))
            for ci in c:
                lines = []
                analytic_account_id = self.env['account.analytic.account'].search([('name','=','CES')])
                # _logger.warning('analytic account es %s', analytic_account_id.id)
                ci.write({'bancard_id': b.id})
                b.write({'campus_id': ci.id})
                # _logger.warning('cursos son %s', ci.curso_ids)
                for curso in ci.curso_ids:
                    # _logger.warning('curso es %s', curso.name)
                    lines.append((0, 0, {
                        ...
                        'analytic_distribution': {analytic_account_id.id: 100.0} if analytic_account_id else {},
