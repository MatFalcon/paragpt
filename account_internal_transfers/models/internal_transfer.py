    ...
    def anular(self):
        for rec in self:
            if rec.state == 'confirmado':
                if rec.cuenta_origen.caja_chica:
                    caja_detalle_egreso = self.env['ruc.caja.detalle.egreso'].search([('transferencia_id','=',rec.id)])
                    if caja_detalle_egreso:
                        caja_detalle_egreso.unlink()
                if rec.check_id:
                    for move in rec.check_id.mapped('deposit_account_move_id'):

                        move.button_cancel()
                        move.unlink(check_move_validity=False)

                    for cheque in rec.check_id:
