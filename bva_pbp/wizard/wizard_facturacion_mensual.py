...
class WizardFacturas(models.TransientModel):
    ...
    def action_facturar(self):
        ...
        USD = self.env['res.currency'].search([('name', '=', 'USD-Compra')])# Deberia se venta pero en el pbp esta con id 2, cuando creo la factura le pongo el id correcto
        facturas = []
        if self.partner_ids:
            for partner in self.partner_ids:
                ...
                if not self.modelos or self.modelos == 'gastos_administrativos':
                    ...
                    if gastos_administrativos:
                        ...
                        if sum_product_gs:
                            #print("SUMA")
                            monto_base_gs = sum_product_gs + (sum_product_gs * 0.1)
                            lineas_facturas_gs.append((0,0,{
                                'partner_id': partner.id,
                                'product_id': p.id,
                                'currency_id': PYG.id,
                                'price_unit': monto_base_gs,
                                'tax_ids': [(6, 0, p.taxes_id.ids)]
                            }))
                        if sum_product_usd:
                            monto_base_usd = sum_product_usd + (sum_product_usd * 0.1)
                            lineas_facturas_usd.append((0,0,{
                                'partner_id': partner.id,
                                'product_id': p.id,
                                'currency_id': USD.id,
                                'price_unit': monto_base_usd,
                                'tax_ids': [(6, 0, p.taxes_id.ids)]
                            }))
                if not self.modelos or self.modelos == 'transferencia_cartera':
                    ...
                    if partner.casa_bolsa:
                        ...
                        if transferencia_cartera:
                            ...
                            print("Trans", transferencia_cartera)
                            if monto_jornal:
                                if len(receptor) + len(emisor) > 0:
                                    lineas_facturas_gs.append((0, 0, {
                                        'partner_id': partner.id,
                                        'product_id': p.id,
                                        'quantity':len(receptor),
                                        'currency_id': PYG.id,
